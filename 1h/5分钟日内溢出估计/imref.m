function [IR, IRplusSE, IRminusSE, se] = imref(y, Beta, IMP, THETA, seIND)
%
%--------------------------------------------------------------------------
...
% IMP = 1 --> shock to stock index
% IMP = 2 --> shock to index future
...
if 1
    figure, plot(IR), legend('Index quantile', 'Future quantile') 
end
%
...
if seIND
    ...
    [se, ~, ~, VC] = standardErrors(y, THETA, Beta, q);

    dVecA = [zeros(4,2),eye(4),zeros(4)];
    dVecB = [zeros(4,2), zeros(4), eye(4)];
    seIRF = zeros(300,2);
