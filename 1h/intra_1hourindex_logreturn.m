function [cell_time_day_1hour,cell_close_day_1hour,logreturn_day_1hour]=intra_1hourindex_logreturn(date_start,date_end,time_all,close_all)
...
for j=1:column
    ...
    for i=1:row
        if time_day{i,j}(4)==9&&time_day{i,j}(5)==30
           cell_time_day_1{k,j}=time_day{i,j};
...
%         end
        ...
        if time_day{i,j}(4)==15&&time_day{i,j}(5)==0
            cell_time_day_1hour{k,j}=time_day{i,j};
            cell_close_day_1hour{k,j}=close_day{i,j};
            k=k+1;
        end        
