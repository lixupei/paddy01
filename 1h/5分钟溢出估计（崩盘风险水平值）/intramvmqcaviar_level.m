function [Q, Beta, fval, exitflag] = intramvmqcaviar_level(y,THETA,St,RSK)
...
for i = 1:2, c(:,i) = intraCAViaR_estim_level(y(:,i), THETA, St(:,i),RSK(:,i)); end
w = c(1,:)'; a = diag(c(2,:)); b = diag(c(3,:)); d=diag(c(4,:));
%部分参数的初始估计值
...
Beta00 = [w; a(:); b(:);d(:)];
empty_location=[4,5,8,9,12,13];

% rng('default')
% rng(10)% set seed
% shock1=zeros(14,nInitialCond);
% off1=(rand(6,nInitialCond)-0.5)/2.5;%生成区间[-0.2,0.2]内的均匀分布随机数
% 
% for k=1:length(empty_location)
%     shock1(empty_location(k),:)=off1(k,:);
% end
% Beta01 = [w; a(:); b(:);d(:)]*ones(1,nInitialCond) + shock1;
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% rng('default')
% rng(20)% set seed
% shock2=zeros(14,nInitialCond);
% off2=randn(6,nInitialCond)/20;
% 
% for k=1:length(empty_location)
%     shock2(empty_location(k),:)=off2(k,:);
% end
% Beta02 = [w; a(:); b(:);d(:)]*ones(1,nInitialCond) + shock2;
% % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % 
...
% Beta0=Beta00;
Beta=zeros(14,size(Beta0,2));
fval=zeros(1,size(Beta0,2));
exittflag=fval;
% Beta0=Beta_all;
parfor i =1:size(Beta0,2)  
    [Beta(:,i),fval(1,i),exitflag(1,i)] = fminsearch('intramqRQobjectiveFunction_level', Beta0(:,i), options, y, THETA, empiricalQuantile, 1,St,RSK); % it does not work with OUT=2 
    for ii = 1:REP
        [Beta(:,i),fval(1,i),exitflag(1,i)] = fminsearch('intramqRQobjectiveFunction_level', Beta(:,i), options, y, THETA, empiricalQuantile, 1,St,RSK); 
