function [cell_time_day_5minute,cell_close_day_5minute,logreturn_day_5minute]=intra_5minutefuture_logreturn(date_start,date_end,time_all,close_all)
...
for j=1:column
    ...
    for i=16:257
        ...
        if time_day{i,j}(4)==14&&ismember(time_day{i,j}(5),[location_choose,59])
            cell_time_day_5minute{k,j}=time_day{i,j};
            cell_close_day_5minute{k,j}=close_day{i,j};
            k=k+1;
        end
%         if time_day{i,j}(4)==15&&ismember(time_day{i,j}(5),location_choose(1))
%             cell_time_day_5minute{k,j}=time_day{i,j};
%             cell_close_day_5minute{k,j}=close_day{i,j};
%             k=k+1;
%         end        
